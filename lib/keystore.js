...
KeyStore.prototype.signTransaction = function (txParams, callback) {
  ...
  if(this.pwDerivedKey){
        var signedTx = signing.signTx(_this, this.pwDerivedKey, rawTx, signingAddress, _this.defaultHdPathString);
    ...
    this.passwordProvider( function (err, password, salt) {
      ...
      KeyStore.deriveKeyFromPassword(password, salt, function (err, pwDerivedKey) {
        if (err) return callback(err);
        var signedTx = signing.signTx(_this, pwDerivedKey, rawTx, signingAddress, _this.defaultHdPathString);
        callback(null, '0x' + signedTx);
      })
    });
  }
 ...
 */
KeyStore.prototype.setPwDerivedKey = function(pwDerivedKey){
  if(pwDerivedKey){
